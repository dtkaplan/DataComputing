...
#' @param start_year the lower end of the time interval for which to download data
#' @param end_year the upper end of the time interval
#'
...
read_stock_prices <- function(symbols='F',
  ...
  for (symbol in symbols) {
    ...
    res <- read.csv(tmpname, stringsAsFactors = FALSE)
    res$company <- symbol
